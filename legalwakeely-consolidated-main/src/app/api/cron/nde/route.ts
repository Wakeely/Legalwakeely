...
async function sendFlagNotification(sb: Supabase, flag: FlagResult, now: Date) {
  ...
  const subjects: Record<number, Record<string, string>> = {
    1: {
      en: `Attention: No lawyer activity on "${caseRow.title}" — Legal Wakeely`,
      ar: `تنبيه: لا يوجد نشاط للمحامي في "${caseRow.title}" — وكيلي القانونى`,
    },
    2: {
      en: `Action Required: Missed deadline on "${caseRow.title}" — Legal Wakeely`,
      ar: `يلزم إجراء: موعد فائت في "${caseRow.title}" — وكيلي القانونى`,
    },
    3: {
      en: `URGENT: Extended silence on "${caseRow.title}" — Legal Wakeely`,
      ar: `عاجل: صمت مطوّل في "${caseRow.title}" — وكيلي القانونى`,
    },
    4: {
      en: `Your lawyer hasn't replied to your message — "${caseRow.title}" — Legal Wakeely`,
      ar: `محاميك لم يردّ على رسالتك في "${caseRow.title}" — وكيلي القانونى`,
    },
    5: {
      en: `Document request unanswered (5 days) — "${caseRow.title}" — Legal Wakeely`,
      ar: `طلب المستند لم يُنفَّذ منذ 5 أيام في "${caseRow.title}" — وكيلي القانونى`,
    },
    6: {
      en: `⚠️ URGENT: Hearing in ≤3 days, no lawyer activity — "${caseRow.title}" — Legal Wakeely`,
      ar: `⚠️ عاجل: جلسة خلال 3 أيام ولا يوجد نشاط للمحامي في "${caseRow.title}" — وكيلي القانونى`,
    },
    7: {
      en: `Reminder: Your evidence vault is empty — "${caseRow.title}" — Legal Wakeely`,
      ar: `تذكير: خزنة الأدلة فارغة في "${caseRow.title}" — وكيلي القانونى`,
    },
  };
  ...
  if (
    ...
    const waBody = isAr
      ? `⚠️ وكيلي القانونى: جلستك القضائية في "${caseRow.title}" خلال 3 أيام أو أقل. لم يُسجَّل أي نشاط من محاميك في الـ 48 ساعة الماضية. سجّل دخولك لاتخاذ إجراء.`
      : `⚠️ Legal Wakeely: Your court hearing for "${caseRow.title}" is in 3 days or less. No lawyer activity has been recorded in the past 48 hours. Log in to take action.`;

    const smsBody = isAr
      ? `وكيلي القانونى: جلستك في "${caseRow.title.slice(0, 30)}" خلال 3 أيام. لا يوجد نشاط من محاميك. سجّل دخولك الآن.`
      : `Legal Wakeely: Court hearing for "${caseRow.title.slice(0, 30)}" in 3 days. No lawyer activity recorded. Log in now.`;
...
function buildAlertEmail(
  ...
  const body = isAr
    ? `تم رصد تنبيه <strong>${ruleName}</strong> على قضيتك <strong>${caseTitle}</strong>. سجّل دخولك إلى وكيلي القانونى لاتخاذ الإجراء المناسب.`
    : `A <strong>${ruleName}</strong> alert has been detected on your case <strong>${caseTitle}</strong>. Log in to Legal Wakeely to take action.`;

  const btnText = isAr ? 'عرض التنبيه' : 'View Alert';
  const disc    = isAr
    ? 'وكيلي القانونى لا تقدم استشارات قانونية. التنبيهات استرشادية فحسب ولا تُعدّ أحكاماً قانونية بالإهمال.'
    : 'Legal Wakeely does not provide legal advice. Alerts are informational only and do not constitute legal findings of negligence.';
