...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // This supersedes any previous active sessions for this user.
      try {
        const { getOrCreateFingerprint } = await import("@/lib/session-enforcement-client");
        const { fingerprint, label } = await getOrCreateFingerprint();
        ...
        await fetch("/api/auth/on-login", {
          ...
          body: JSON.stringify({ session_id: sessionId, expires_at: expiresAt }),
        });
      } catch {
        // Non-fatal — session enforcement is best-effort
      }
  ...
  const handleGoogle = async () => {
    // Store fingerprint in sessionStorage so the auth callback can read it
    try {
      const { getOrCreateFingerprint } = await import("@/lib/session-enforcement-client");
      const { fingerprint, label } = await getOrCreateFingerprint();
      sessionStorage.setItem("lw.pending-fp", fingerprint);
      sessionStorage.setItem("lw.pending-label", label);
    } catch {
      // ignore
    }
