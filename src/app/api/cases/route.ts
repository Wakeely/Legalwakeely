...
const createCaseSchema = z.object({
  ...
  case_type: CASE_TYPE_SCHEMA,
  jurisdiction: z.string().trim().max(200).optional().default(''),
  city: z.string().trim().max(200).optional().nullable().default(null),
  description: z.string().trim().max(10_000).optional().nullable().default(null),
  lawyer_name: z.string().trim().max(200).optional().nullable().default(null),
  lawyer_bar_number: z.string().trim().max(100).optional().nullable().default(null),
  lawyer_phone: z.string().trim().max(50).optional().nullable().default(null),
  lawyer_email: z.string().trim().email().max(200).optional().nullable().default(null),
  deadlines: z
    .array(
      z.object({
        title: z.string().trim().min(1).max(300),
        due_date: z.string().trim().min(1).max(50),
        type: z.enum(['court', 'submission', 'internal']).optional().default('court'),
      }),
    )
    .max(50)
    .optional()
    .default([]),
  draft_id: z.string().uuid().optional().nullable().default(null),
  draft_data: z.record(z.unknown()).optional(),
  draft_step: z.number().int().min(0).max(20).optional(),
});
