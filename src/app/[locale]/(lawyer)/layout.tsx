...
import { FolderOpen, Settings, FileText, Building2 } from 'lucide-react';
...
export default async function LawyerLayout({ children }: { children: React.ReactNode }) {
  ...
  // next/navigation redirect — full path is correct here (not next-intl Link)
  ...
  const navItems = [
    { href: '/lawyer/cases', icon: FolderOpen, label: isRTL ? 'قضاياي' : 'My Cases' },
    { href: '/lawyer/invoices', icon: FileText, label: isRTL ? 'الفواتير' : 'Invoices' },
    { href: '/lawyer/firm', icon: Building2, label: isRTL ? 'المكتب' : 'Firm' },
    { href: '/settings', icon: Settings, label: isRTL ? 'الإعدادات' : 'Settings' },
  ] as const;
    ...
    <div className="flex min-h-screen bg-background">
      <aside className="hidden lg:flex flex-col w-64 border-e border-border bg-card shrink-0">
        <div className="flex items-center gap-2 px-6 py-5 border-b border-border">
          <img
            src="/logo.png"
            alt={isRTL ? 'وكيلي القانونى' : 'Legal Wakeely'}
