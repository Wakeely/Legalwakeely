...
export function buildLetterBody(
  ...
  const footer_en = `\n\nYours sincerely,\n${fields.client_name}${fields.client_phone ? `\nPhone: ${fields.client_phone}` : ''}${fields.client_email ? `\nEmail: ${fields.client_email}` : ''}\n\n---\nGenerated by Legal Wakeely Platform · Not legal advice · ${new Date().toISOString().split('T')[0]}`;
  const footer_ar = `\n\nمقدمه بكل احترام،\n${fields.client_name}${fields.client_phone ? `\nهاتف: ${fields.client_phone}` : ''}${fields.client_email ? `\nبريد: ${fields.client_email}` : ''}\n\n---\nأُنشئ بواسطة منصة وكيلي القانونى · وكيلي القانونى لا تقدم استشارات قانونية · ${new Date().toISOString().split('T')[0]}`;
