        ...
        <div className="px-6 py-5 space-y-4">
          {step === 'form' ? (
            ...
            <>
              ...
              {email && (
                <a
                  href={`mailto:${email}?subject=${encodeURIComponent(isRTL ? `دعوة وكيلي القانونى — ${caseTitle}` : `Legal Wakeely Invite — ${caseTitle}`)}&body=${encodeURIComponent(link)}`}
              ...
              <a
                href={`https://wa.me/?text=${encodeURIComponent(
                  isRTL
                    ? `دعوة وكيلي القانونى للقضية "${caseTitle}":\n${link}`
                    : `Legal Wakeely invite for case "${caseTitle}":\n${link}`
                )}`}
                target="_blank"
                rel="noopener"
